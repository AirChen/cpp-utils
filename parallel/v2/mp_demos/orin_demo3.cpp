...
class MyBenchmark : public benchmark::Fixture {
 public:
  void SetUp(const ::benchmark::State& state) override {
    std::call_once(flag, [this, &state]() {
      ...
      sums_atomic_v2 = std::vector<PaddedCounterLock>(state.threads());
  ...
  std::vector<PaddedCounterLock>& GetSumsAtomicV2() { return sums_atomic_v2; }
 ...
 private:
  ...
  std::vector<PaddedCounterLock> sums_atomic_v2;
...
BENCHMARK_DEFINE_F(MyBenchmark, MultiThreadedWorkAtomic)(benchmark::State& state) {
  ...
  for (auto _ : state) {
    ...
    for (int i = start; i < end; i++) {
      ...
      if (inst.count(i) > 0) {
        std::lock_guard<folly::SpinLock> lg(m);
